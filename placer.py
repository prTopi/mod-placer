...
class ChooseConfig(QDialog):
	...
	def refresh(self, conf=''):
		...
		[self.configList.addItem(conf) for conf in listdir() if conf.endswith('.json')]
	...
	def editConfig(self, name):
		...
		self.dialog = EditDialog({'File Name': name[:-5], 'Nexus Game': game, 'Mods Directory': mods, 'Data Path': data, 'Plugins.txt File': plugins, 'Plugins.txt Line Prefix': prefix}, self)
		if self.dialog.exec_():
			name, game, mods, data, plugins, prefix = self.dialog.getValues()
			if not name.endswith('.json'):
				name += '.json'
			if self.configList.currentText() != name and name in listdir():
				QMessageBox.warning(self, 'File already exists', 'Mod config with that name already exists.', QMessageBox.Ok)
				name = self.configList.currentText()
				if not name.endswith('.json'):
					name += '.json'
			config['Settings'] = {'data': path.realpath(data), 'game': game, 'mods': path.realpath(mods), 'plugins': path.realpath(plugins), 'pluginpref': prefix}
...
class ModPlacer(QWidget):
	def __init__(self, conf, api, parent):
		...
		self.api = api
		self.parent = parent
		self.headers = {'User-Agent': 'ModPlacer/{} ({}) Python/{}'.format(__version__, platform(), python_version()), 'apikey': api}
		with open(conf) as f:
			self.config = load(f)
	...
	def refreshMods(self, save=True):
		...
		if save:
			self.saveConfig()
		self.modList.clear()
		[self.addModItem(*self.config['Mods'][mod]) for mod in self.config['Mods']]
		[self.addModItem(mod) for mod in listdir(self.mods)]
		self.loadList.clear()
		[self.addLoadItem(*self.config['LoadOrder'][esp]) for esp in self.config['LoadOrder']]
		if path.isdir(self.data):
	...
	def saveConfig(self):
		if self.loadList.count() + 5 < len(self.config['LoadOrder']):
			if QMessageBox.warning(self, 'Warning', 'Load order count varies greatly from config count.\nDo you want to skip saving?', QMessageBox.Yes | QMessageBox.No) == QMessageBox.Yes:
				return
		self.config['Mods'] = {x: (self.modList.item(x).text(), self.modList.item(x).checkState()) + self.modList.item(x).data(Qt.UserRole) for x in range(self.modList.count())}
		self.config['LoadOrder'] = {x: (self.loadList.item(x).text(), self.loadList.item(x).checkState()) for x in range(self.loadList.count())}
